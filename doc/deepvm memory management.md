...
2. 将全局可用内存空间（`addr` 处的八字节）设为 `n - 96`
3. 将跳表首块地址（`addr + 8` 处的八字节）设为 `addr + 8`
4. 将剩余块（Remainder Block）首地址（`addr + 16` 处的八字节）设为 `addr + 96`
5. 将剩余块（Remainder Block）尾地址（`addr + 24` 处的八字节）设为 `addr + n`
6. 将 Fast Bin Array （`addr + 32` – `addr + 95` 共 64 字节）每八字节为一组，均设为自身的地址
   - 例如，`addr + 32` 起始的八字节应设为 `addr + 32`；最后一个 `addr + 88` 起始的八字节应设为 `addr + 88`
7. 将剩余块的头部（`addr + 96` – `addr + 99`）初始化
