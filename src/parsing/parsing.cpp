...
namespace dp {
    namespace internal {
        ...
        antlrcpp::Any Parser::visitExpressionStatement(DLParser::ExpressionStatementContext* context) {
            ...
            } else if(context->IF_SYMBOL()) {  //condition
                ConditionExStmt* ecxSt = new ConditionExStmt();
                ecxSt->expr = std::unique_ptr<Expression>(static_cast<Expression*>(visit(context->unblockExpression())));
                ecxSt->cond = static_cast<ConditionElem*>(visit(context->conditionElem()));
                return ecxSt;
