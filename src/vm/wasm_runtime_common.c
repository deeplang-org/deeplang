...
#define n_fps n_ints
	...
	//debug ("fps=%p, ints=%p, stacks=%p", fps, ints, stacks);
    ...
    if (result_count == 0) {
        ...
        switch (func_type->types[func_type->param_count]) {
            case VALUE_TYPE_I32:
				//debug ("funp=%p，argv=%p,n_ints=%d",func_ptr,argv1,n_ints);
				/* 64bit pointer cut */
                argv_ret[0] = (uint32)invokeNative_Int32(func_ptr, ints, n_ints);
                break;
            case VALUE_TYPE_I64:
                PUT_I64_TO_ADDR(argv_ret, invokeNative_Int64(func_ptr, argv1, n_stacks));
                break;
